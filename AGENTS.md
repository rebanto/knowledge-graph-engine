...
This is the canonical agent-context file for this project. It is read by Codex
directly (AGENTS.md) and by Claude Code via a one-line `@AGENTS.md` import in
CLAUDE.md. Keep all project context here — do not duplicate it into CLAUDE.md.
...
| LLM API              | Google Gemini API via `google-genai` (`gemini-flash-lite-latest`, set by `LLM_MODEL`) | Phase 1 | Entity extraction, routing, synthesis, OCR — all in `backend/core/llm_client.py` |
| Embedding model      | Local `sentence-transformers` (`all-MiniLM-L6-v2`) | Phase 1 | Runs in-process, no API key; used for Chroma vectors + entity resolution |
...
```
/
├── AGENTS.md                  ← Canonical agent context (this file). Read every session.
├── CLAUDE.md                  ← One-line `@AGENTS.md` import so Claude Code picks it up.
...
```
GEMINI_API_KEY=           # the only required key; without it every LLM call fails
LLM_MODEL=gemini-flash-lite-latest   # Gemini model for all text/JSON/stream/OCR calls
# Embeddings run locally via sentence-transformers (all-MiniLM-L6-v2) — no API key needed
